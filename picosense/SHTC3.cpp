...
bool SHTC3_Read(uint8_t *pData, uint8_t Len) {
  DEV_I2C_Read_nByte(SENSOR_I2C_PORT, SHTC3_I2C_ADDR, pData, Len);
  return true;
}
...
bool SHTC3_Measurement(float *temp, float *hum) {
  ...
  // uint16_t command
  // =SHTC3_MEAS_ALL[SHTC3_STRETCH_MEAS][SHTC3_LOWPOWER_MEAS][SHTC3_HUM_FRIST_MEAS];
  ...
  if (temp_data_crc == buffer[3 * (SHTC3_HUM_FRIST_MEAS + 1) - 1] &&
      hum_data_crc == buffer[3 * (2 - SHTC3_HUM_FRIST_MEAS) - 1]) {
    temp_data = (buffer[SHTC3_HUM_FRIST_MEAS * 3] << 8) +
                buffer[SHTC3_HUM_FRIST_MEAS * 3 + 1];
    hum_data = (buffer[3 - SHTC3_HUM_FRIST_MEAS * 3] << 8) +
               buffer[4 - SHTC3_HUM_FRIST_MEAS * 3];
    *temp = (temp_data * 175.0) / (1 << 16) - 45;
    *hum = (hum_data * 100.0) / (1 << 16);
    return true;
  }
  printf("temp_data_crc = %x ,get_crc = %x\r\n", temp_data_crc,
         buffer[3 * (SHTC3_HUM_FRIST_MEAS + 1) - 1]);
